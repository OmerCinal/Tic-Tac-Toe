...
class Game:
    ...
    def drawboard(self, n):
        ...
        for i in range(n):
            board.create_text(space*(i+2), space, text=str(i), anchor=W, tag="coord")
            board.create_text(space, space*(i+2), text=str(i), anchor=W, tag="coord")
            
        for i in range(n+1):
            board.create_line(start, start + space*i, start + space*n, start + space*i, tag="line")
            board.create_line(start + space*i, start, start + space*i, start + space*n, tag="line")
    ...
    def start(self, event):
        ...
        score1, score2, steps = self.playOnce(shape1, shape2)
        self.drawboard(self.size)
    ...
    def playOnce(self, shape1, shape2):
        play1 = self.player1.play
        play2 = self.player2.play

        step = 1
        steps = []
        while not self.endgame(self.board):
    ...
    def test(self, event):
        ...
        scores1 = []
        scores2 = []
        for step in xrange(testrange):
            self.board = [ [0]*self.size for _ in range(self.size) ]
            score1, score2, _ = self.playOnce(shape1, shape2)
            scores1.append(score1)
            scores2.append(score2)

            self.canvas.delete(ALL)
            self.canvas.create_text(300, 100, text=str(step+1)+" / "+str(testrange), font=50)
            self.canvas.update()
    ...
    def drawChains(self, event):
        ...
        for p in self.chains:
            for x1, y1, x2, y2 in self.chains[p]:
                self.canvas.create_line(start+y1*space, start+x1*space, start+y2*space ,start+x2*space, fill=colors[p], tag="chain", state=NORMAL, width=2.0)
